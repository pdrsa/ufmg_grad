...
if __name__ == '__main__':
    ...
    while(counter < limit and not frontier.empty()):
        ...
        # This can be uncommented to show an ETA
        ...
        try:
            ...
            i     = 0
            shift = 0
            while len(url_batch) < NUM_THREADS and counter <= limit:
                if(frontier.empty()):
                    break
        ...
        # Shifting and counting number of processed pages
        u = [x[0] for x in urls]
        v = [x[1] for x in urls]
        resp = list(map(visit, u, v))
        counter += len(resp)
        shift   += len(resp)
