...
int main(){ _
	...
	while(cin >> n >> m){
		...
		for(int i = 1; i <= n; i++){
			int f; cin >> f;
			flow.add_edge(i, sink, f);
			edges.pb(f);
		}
		vector<int> read(m); for(int &i: read) cin >> i;
		...
		for(int i = 0; i < m; i++){
			...
			int cat = n+i+1;
			flow.add_edge(source, cat, f);
			
			while(read[i]--){
				int vodka; cin >> vodka;
				flow.add_edge(cat, vodka, INF);
			}
		}
		
		cout << sum - flow.max_flow(source, sink) << endl;
	}
	exit(0);
}
