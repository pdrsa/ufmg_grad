...
int32_t main(){ _
	int n, q; cin >> n >> q;
	vector<int> a(n), sete(n), treze(n);
	for(int i = 0; i < n; i++){
		cin >> a[i];
		if(a[i] == 13) a[i] = 0, treze[i] = 1;
		if(a[i] == 7)  a[i] = 0, sete[i]  = 1;
	}
	Segtree Seg, Sete, Treze;
	Seg.build(a), Sete.build(sete), Treze.build(treze);

	while(q--){
		int qt, l, r; cin >> qt >> l >> r;
		if(qt == 2){
			int men, mai; cin >> men >> mai;
			if(men == 7 and mai == 13)
			seg.update(l, r);
		}
		else if(qt == 3){
			...
			ans += 7 * Sete.query(l, r);
			ans += 13 * Treze.query(l, r); 
			cout << ans << endl;
		}
	}
	exit(0);
